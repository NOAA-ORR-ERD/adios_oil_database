...
if [ “$REFRESH_INTERNAL_DB” == “true” ]; then
    ...
    echo ">>> merging production into under_review"
    git checkout production -- validation/validation_by_*
    git commit -a -m "pulling validation from production branch"
    git merge -m "merging in changes from production branch" production
    ...
    echo ">>> committing changes (if any) to repo"
