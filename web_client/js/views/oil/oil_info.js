define([
    ...
    'model/oil'
            ...
            OilInfoTemplate, NotFoundTemplate, OilModel) {
    'use strict';
    var oilInfoView = Backbone.View.extend({
        ...
        initialize: function(options) {
            var _thisClass = this
            var oilModel = new OilModel(options);

            oilModel.fetch()
            .then(function() {
                console.log('oil fetch success');
                _thisClass.model = oilModel;
                _thisClass.render();
            })
            .fail(function(reason) {
                console.log('oil fetch failed!!!');
                _thisClass.model = null;
                _thisClass.render();
            });

            if (!_.isUndefined(oilModel.imported)) {
                console.log('imported ID: ' + oilModel.imported);
            }
        },
        ...
        processOilNode: function(parentAttr, parentObj, key) {
                     ...
                     key === 'oil_water_interfacial_tension_n_m')
            {
                ...
                // OK, we will bypass this for now.
