'''
OilProps class which serves as a wrapper around the Oil Model class
...
class OilProps(ImportedRecordWithEstimation):
    '''
    Class which:
    ...
    def to_json(self):
        '''
            Return the underlying oil object JSON content
        '''
        return self.record.to_son().to_dict()
    ...
    def __eq__(self, other):
        '''
            ...
            However, PyGnome initializes two OilProps object when invoked from
            the WebGnomeClient, there is an Model object embedded in
            .record which maybe different. To avoid comparing the Model
            object that is part of the raw oil record, this works as follows:

            1. check if self.__dict__ == other.__dict__
            2. if above fails, then check if the to_json() for both OilProps
               objects match. This assumes that both objects contain to_json()

            TODO: I am not convinced this will work with MongoDB objects in
                  all cases.  .to_json() is also maybe a bit heavy for this
                  purpose.
        '''
    ...
    def __deepcopy__(self, memo):
        '''
            The _r_oil object should not be copied - it should just be
            referenced to create the OilProps copy. The database record itself
            does not need to be a deepcopy - both OilProps objects can
            reference the same database record
        '''
    ...
    def _init_sara(self):
        '''
            Initialize self._sara as a numpy array. The information is
            structured in increasing boiling points as:
