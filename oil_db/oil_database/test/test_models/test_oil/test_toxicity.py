'''
    Test our main Toxicity model class
'''
import pytest

from pydantic import ValidationError

from oil_database.models.oil import Toxicity
...
class TestToxicity():
    ...
    def test_init_required(self, tox_type, species):
        if tox_type is None and species is None:
            ...
            obj = Toxicity(species=species)
        elif species is None:
            obj = Toxicity(tox_type=tox_type)
        else:
            obj = Toxicity(tox_type=tox_type, species=species)

        assert obj.tox_type == str(tox_type)
        assert obj.species == str(species)

    def test_init_defaults(self):
        obj = Toxicity(tox_type='EC', species='Species')
