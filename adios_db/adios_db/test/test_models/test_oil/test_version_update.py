...
def test_new_version_still_works():
    """
    if we try to create an Oil object from a JSON file that is newer than the code

    it should try to read it anyway, and only fail if it can't read it
...
def test_newer_version_cant_load():
    ...
    with pytest.raises(VersionError):
        this_oil = oil.Oil.from_file(DATADIR / "high_version_changed.json")
