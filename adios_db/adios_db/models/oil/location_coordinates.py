    ...
    def __post_init__(self):
        ...
        if self.type == "Point":
            ...
            try:
                ...
                raise ValueError("Point type coordinates must be a "
                                 "pair of numbers")
            ...
            try:
                ...
                raise ValueError("Polygon coordinates must be a "
                                 "list of lists of points")

            if len(ring) < 4:
                raise ValueError("Polygon coordinates must be a least "
                                 "four points")

            if ring[0] != ring[-1]:
                raise ValueError("First and last point of a polygon "
                                 "must be equal")

            for p in ring:
                if len(p) != 2:
                    raise ValueError("All points of a polygon must have "
                                     "two coordinates")
        else:
            raise ValueError('LocationCoordinates type must be either '
                             '"Point" or "Polygon"')
