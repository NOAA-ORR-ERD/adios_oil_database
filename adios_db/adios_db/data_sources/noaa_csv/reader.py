    ...
    def load_from_csv(self, infile):
        with open(infile, newline='') as csvfile:
            ...
            if header[0].lower() == "ADIOS Data Model Version".lower():
                ...
                raise ValueError("First line doesn't match -- are you sure this is a"
                                 "standard ADIOS CSV file?\n"
                                 f"First line of file: {header}")
            if data_model_version != ADIOS_DATA_MODEL_VERSION:
                raise ValueError(f"Version mismatch -- this file is: {data_model_version}\n"
                                 f"The code version is: {ADIOS_DATA_MODEL_VERSION}")
            ...
            for line in reader:
                if line[0].lower().strip() == "Subsample Metadata".lower().strip():
